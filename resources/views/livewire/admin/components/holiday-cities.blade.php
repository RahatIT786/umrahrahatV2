<div class="card">
    <div class="card-body">
        ...
        <div class="d-flex align-items-center justify-content-between mb-3">
            <h6 class="card-title mb-0">Holiday Cities Management</h6>
   ...
   class="text-danger" data-bs-placement="bottom" title="Create Car" data-bs-toggle="modal" data-bs-target="#exampleVerticallycenteredModal">
    <i class="bx bx-plus me-1"></i> Create  Holiday cities
</a>
        ...
        <div class="d-flex align-items-center mb-3">
            ...
            <form class="ms-auto position-relative" wire:submit.prevent="">
                <input type="text" class="form-control" placeholder="Search Holiday Cities" wire:model.live="search">
            </form>
        </div>
        ...
        <div class="table-responsive mt-3">
            <table class="table align-middle">
               ...
               <tbody>
                    @foreach ($holidayCities as $index => $holidayCity)
                        <tr>
                            <td>{{  $index + 1 }}</td>
                            <td>{{ $holidayCity->Holiday_city }}</td>
                            <td class="text-center">
                                <a class="text-primary" wire:click.prevent="edit({{ $holidayCity->id }})" data-bs-toggle="modal"  title="Edit">
                                    <i class="bi bi-pencil-fill"></i>
                                </a>
                            </td>
                            <td class="text-center">
                                <a wire:click="confirmDelete({{ $holidayCity->id }})" class="text-danger" data-bs-placement="bottom" title="Delete" data-bs-toggle="modal" data-bs-target="#exampleVerticallycenteredModal1">
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Are you sure you want to delete this Holiday City?
                    </div>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalCenterTitle">
                            {{ $showEditModal ? 'Edit Holiday Cities' : 'Add Holiday Cities' }}
                        </h5>
                    ...
                    <form wire:submit.prevent="{{ $showEditModal ? 'update' : 'save' }}" onsubmit="setTimeout(() => location.reload(), 1000); return false;">
                        <div class="modal-body">
                            <label for="Holiday_city" class="form-label">Holiday City</label>
                            <input type="text" class="form-control" id="hotel_city" wire:model="Holiday_city" required>
                            @error('Holiday_city') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>
